...
pub fn type_vectorized_definitions<D: Dialect>(
    ...
    for item in items.iter() {
        ...
        if size > 1 && !item.native {
            // Non-native: generate struct (CUDA/HIP compatible)
            write!(
                f,
                "
struct __align__({alignment}) {item} {{"
    ...
    {elem} i_{i};"
        ...
        // When item.native is true (e.g. Metal float4), no struct needed —
        // the type is a built-in vector type (float2, float4, half4, etc.)
    }
    Ok(())
}
