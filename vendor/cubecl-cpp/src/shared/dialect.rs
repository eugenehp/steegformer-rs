...
pub trait DialectTypes<D: Dialect> {
    ...
    /// Whether this dialect supports native vector types (float2, float4, etc.)
    /// When true, vectorized items use native types instead of struct wrappers,
    /// enabling hardware-accelerated vectorized operations.
    fn supports_native_vec() -> bool { false }
    fn compile_elem(
        f: &mut std::fmt::Formatter<'_>,
        elem: &Elem<D>,
        word: bool,
