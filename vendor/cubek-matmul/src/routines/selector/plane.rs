...
pub fn infer_blueprint_plane<TMM: TileMatmulFamily, R: Runtime>(
    ...
    let row_count = options.row_count.unwrap_or_else(|| {
        ...
        // For Metal with 8×8 tiles: use precision_factor=1 to maximize output tile size.
        // MPS uses 64×64 output tiles (8×8 array of 8×8 WMMA tiles = 64 tiles per workgroup).
        // Original factor=2 for f32 was too conservative, producing 512-element output tiles
        // vs MPS's 4096-element tiles (8× less work per dispatch).
